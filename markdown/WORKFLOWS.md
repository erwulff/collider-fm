...
Probe train/val events are disjoint consecutive slices from the start of `evaluation.val_split` (defaults: 2000 train / 500 val), so neither was seen by pretraining. Sizing knobs: `evaluation.probe_train_events`, `probe_val_events`, `probe_max_points` (per-point row cap per split), `probe_max_calo_hits` (per-event forward cap, off by default — the energy target covers the whole event, so truncating the input would bias the energy probe); optimizer knobs: `probe_seg_epochs`, `probe_energy_epochs`, `probe_batch_size`, `probe_lr` (single-head), `probe_lr_list` (Panda-style LR sweep — `"1e-5,...,0.1"` comma-separated), `probe_weight_decay`, `probe_early_stop_patience`, `probe_every_n_epochs` (in-training probes at rank 0). Results land under `"probes"` in `summary.json` and as `probes/{trained,random_init,raw_input}/...` + `probes_report.txt` in the eval dir. Run once with the checkpoint; the trained-vs-random delta is the pretraining track.
...
- `probes/random_init/...` and `probes/raw_input/...` — the same three PNGs for the baselines when enabled.
